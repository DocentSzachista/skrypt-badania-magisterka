from configuration import Config, prepare_counted_values_output_dir
import json
import pandas as pd
...
def handle_mixup(
        augumentation: MixupAugumentation, loaded_config: Config, k_nearest_neighbours: NearestNeightboursCount,
    ...
    for chosen_class in augumentation.classes:
        base_dir = "{}/dataframes/mixup_to_{}".format(augumentation.template_path, chosen_class)
        ...
        acc_wide = {k: [] for k in loaded_config.labels}
        ...
        for step in iterator:
            ...
            confussion.make_noise_wide_statistic(
                y_true, y_pred, list(loaded_config.labels),
                acc_wide, filename=output_path_matrix)
...
def handle_noise(
        augumentation: NoiseAugumentation, loaded_config: Config, k_nearest_neighbours: NearestNeightboursCount,
        cosine: CosineDistance, mahalanobis: MahalanobisDistance, euclidean: EuclidianDistance):
    print("perform calculations for {}".format(augumentation.name))
    ...
    for step in iterator:
        ...
        y_true = df['original_label'].values
        confussion.make_noise_wide_statistic(
            y_true, y_pred, loaded_config.labels, acc_wide, filename=output_path_matrix)
        neighbours = k_nearest_neighbours.count_neighbours(df)
        dist_cosine = cosine.count_distance(df)
        dist_mahalanobis = mahalanobis.count_distance(df).T
        dist_euclidean = euclidean.count_distance(df)
...
def make_calculations(loaded_config: Config):
    ...
    for augumentation in loaded_config.augumentations:
        ...
        for step in iterator:
            ...
            y_true = df['original_label'].values
            confussion.make_noise_wide_statistic(
                y_true, y_pred, loaded_config.labels, acc_wide,
