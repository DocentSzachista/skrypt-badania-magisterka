...
def handle_mixup(
    ...
    for chosen_class in augumentation.classes:
        base_dir = "{}/dataframes/mixup_to_{}".format(BASE_PATH.format(loaded_config.model,
                                                      loaded_config.tag, augumentation.name), chosen_class)
        ...
        for step in iterator:
            ...
            confussion.make_noise_wide_statistic(
                y_true, y_pred, list(LABELS_CIFAR_10.keys()),
...
def make_calculations(loaded_config: Config):
    ...
    train['features'] = train['features'].apply(
        lambda x: (x - np.min(x)) / (np.max(x) - np.min(x)))  # data standarization
